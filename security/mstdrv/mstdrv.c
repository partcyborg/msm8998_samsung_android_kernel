...
static void update_policy_online(void)
{
	...
	for(i = 0; i < num_clusters; i++) {
		pr_info("Updating policy for CL%d\n", i);
		cpufreq_update_policy(cpumask_first(&sched_cluster[i]->cpus));
	}
	put_online_cpus();
}
...
{ 
	...
	/* Set the min freq for all CPUs in the system */ 
	pr_debug("Setting SS boost min for all CPUs\n"); 

	for(i = 0; i < num_clusters; i++) {
		cpu_policy = cpufreq_cpu_get(cpumask_first(&sched_cluster[i]->cpus));
		cluster_freq[i] = cpu_policy->user_policy.min; // save min freq 

		/* set new freq */ 
		cpu_policy->user_policy.min = cpu_max_freq(cpumask_first(&sched_cluster[i]->cpus)); 
		pr_info("[%s]CL[%d]CPU[%d]: cur_min=%u, new_min=%u, max=%u\n", __func__, i, cpumask_first(&sched_cluster[i]->cpus),
				cluster_freq[i], cpu_policy->user_policy.min , cpu_max_freq(cpumask_first(&sched_cluster[i]->cpus))); 
	} 
...
{ 
	...
	for(i = 0; i < num_clusters; i++) { 
		cpu_policy = cpufreq_cpu_get(cpumask_first(&sched_cluster[i]->cpus));
		cpu_policy->user_policy.min = cluster_freq[i]; // restore old min freq 
		pr_info("[%s]CL[%d]CPU[%d]: cur_min=%u, new_min=%u, max=%u\n", __func__, i, cpumask_first(&sched_cluster[i]->cpus),
			   cluster_freq[i], cpu_policy->user_policy.min , cpu_max_freq(cpumask_first(&sched_cluster[i]->cpus))); 
	}
...
static void of_mst_hw_onoff(bool on){
	...
	if(on) {
		...
		printk("%s : boost enable for high CPU freq", __func__);
		boost_enable();
